...
For anything outside this lifecycle, `AS.custom('uploading')` accepts any string.
...
AS.destroy(); // resolves all pending wait()s, clears all listeners, blocks all further changes
```
...
- `destroy()` resolves all pending `wait()` calls immediately, then clears all listeners.
